...
const CAMINHO_MATERIAIS = ASSETS_JOGO.base;
  ...
  ponte1: [
    {
      ...
      objetos: [
        { id: "madeira", nome: "Madeira", imagem: caminhoAsset("madeira.png"), correta: true },
        { id: "corda", nome: "Corda", imagem: caminhoAsset("corda.png"), correta: true },
        { id: "pregos", nome: "Pregos", imagem: caminhoAsset("pregos.png"), correta: true },
        { id: "bola", nome: "Bola", imagem: caminhoAsset("bola.png"), correta: false },
        { id: "maca", nome: "Maçã", imagem: caminhoAsset("maca.png"), correta: false },
        { id: "pena", nome: "Pena", imagem: caminhoAsset("pena.png"), correta: false }
      ]
    },
    {
      tentativa: 2,
      ...
      objetos: [
        { id: "troncos", nome: "Troncos", imagem: caminhoAsset("troncos.png"), correta: true },
        { id: "martelo", nome: "Martelo", imagem: caminhoAsset("martelo.png"), correta: true },
        { id: "parafusos", nome: "Parafusos", imagem: caminhoAsset("parafusos.png"), correta: true },
        { id: "chapeu", nome: "Chapéu", imagem: caminhoAsset("chapeu.png"), correta: false },
        { id: "peixe", nome: "Peixe", imagem: caminhoAsset("peixe.png"), correta: false },
        { id: "flores", nome: "Flores", imagem: caminhoAsset("flores.png"), correta: false }
      ]
    },
    {
      tentativa: 3,
      ...
      objetos: [
        { id: "vigas", nome: "Vigas", imagem: caminhoAsset("vigas.png"), correta: true },
        { id: "alicate", nome: "Alicate", imagem: caminhoAsset("alicate.png"), correta: true },
        { id: "correntes", nome: "Correntes", imagem: caminhoAsset("correntes.png"), correta: true },
        { id: "bolo", nome: "Bolo", imagem: caminhoAsset("bolo.png"), correta: false },
        { id: "almofada", nome: "Almofada", imagem: caminhoAsset("almofada.png"), correta: false },
        { id: "guarda_chuva", nome: "Guarda-chuva", imagem: caminhoAsset("guarda_chuva.png"), correta: false }
      ]
    }
  ]
};
...
function limparTodaMemoriaCaixasPerdidas() {
...
window.limparMemoriaCaixasPerdidas = limparTodaMemoriaCaixasPerdidas;
...
function limparMemoriaTentativaCaixasPerdidas(ponte, tentativa) {
  if (!tentativa) return;
  const chave = chaveMemoriaCaixasPerdidas(ponte, tentativa);
  delete memoriaCaixasPerdidas[chave];
  const memoriaSessao = lerMemoriaCaixasSessao();
...
function criarEstrelasErros(erros) {
  return Array.from({ length: MAX_ERROS_CAIXAS_PERDIDAS })
    .map((_, indice) => {
      ...
      return `<span class="estrela-erro coracao-erro ${classe}">❤️</span>`;
    })
    .join("");
}
...
function abrirCaixaMiniJogo(botao, estadoMiniJogo) {
  ...
  if (imagemCaixa) {
    imagemCaixa.src = caminhoAsset("caixa-aberta.png");
  }
...
function mostrarVitoriaCaixasPerdidas(estadoMiniJogo) {
  ...
  limparMemoriaTentativaCaixasPerdidas(ponte, tentativa);
...
const CAMINHO_PONTE1 = ASSETS_JOGO.base;

const ESCALA_VISUAL_PONTE1 = 0.68;
const DESLOCAMENTO_VISUAL_PONTE1 = (100 - (ESCALA_VISUAL_PONTE1 * 100)) / 2;

function calcularValorVisualPonte1(valorOriginal) {
  return DESLOCAMENTO_VISUAL_PONTE1 + (valorOriginal * ESCALA_VISUAL_PONTE1);
}

function obterDadosVisuaisPecaPonte1(peca) {
  return {
    alvoX: calcularValorVisualPonte1(peca.alvoX),
    alvoY: calcularValorVisualPonte1(peca.alvoY),
    largura: peca.largura * ESCALA_VISUAL_PONTE1
  };
}

const pecasReconstrucaoPonte1 = [
  // As coordenadas alvo originais continuam a corresponder à imagem base da ponte.
  // A escala visual é aplicada de forma centralizada para reduzir a ponte inteira
  // e manter as peças proporcionais ao espaço que ocupam quando encaixam.
  { id: "peca1", imagem: "assets/peca1.png", alvoX: 0, alvoY: 35.22, largura: 24.7, inicioX: 5, inicioY: 13 },
  { id: "peca2", imagem: "assets/peca2.png", alvoX: 26.56, alvoY: 70.58, largura: 32.45, inicioX: 14, inicioY: 72 },
  { id: "peca3", imagem: "assets/peca3.png", alvoX: 30.6, alvoY: 41.16, largura: 15.17, inicioX: 84, inicioY: 14 },
  { id: "peca4", imagem: "assets/peca4.png", alvoX: 37.48, alvoY: 14.09, largura: 21.59, inicioX: 40, inicioY: 6 },
  { id: "peca5", imagem: "assets/peca5.png", alvoX: 54.9, alvoY: 40.88, largura: 16.82, inicioX: 82, inicioY: 50 },
  { id: "peca6", imagem: "assets/peca6.png", alvoX: 60.73, alvoY: 2.9, largura: 6.42, inicioX: 7, inicioY: 59 },
  { id: "peca7", imagem: "assets/peca7.png", alvoX: 70.79, alvoY: 0, largura: 8.01, inicioX: 72, inicioY: 8 },
  { id: "peca8", imagem: "assets/peca8.png", alvoX: 75.63, alvoY: 21.82, largura: 8.21, inicioX: 60, inicioY: 66 },
  { id: "peca9", imagem: "assets/peca9.png", alvoX: 87.68, alvoY: 15.19, largura: 12.32, inicioX: 76, inicioY: 75 }
];
    ...
    conteudo: `
      <div class="ponte1-fluxo ponte1-introducao">
        ...
        <div class="ponte1-imagem-wrap ponte1-imagem-original">
          <img src="assets/original-sem-ponte.png" alt="Mapa com a Ponte 1 em falta">
...
function mostrarReconstrucaoPonte1() {
  const pecasHTML = pecasReconstrucaoPonte1
    .map((peca) => {
      const dadosVisuais = obterDadosVisuaisPecaPonte1(peca);
      ...
      return `
        <button
          ...
          class="ponte1-peca"
          data-id="${peca.id}"
          style="left:${peca.inicioX}%; top:${peca.inicioY}%; width:${dadosVisuais.largura}%;"
          aria-label="Peça da ponte"
        >
          <img src="${normalizarCaminhoAsset(peca.imagem)}" alt="">
        </button>
      `;
    })
    .join("");
  ...
  abrirModal({
    ...
    conteudo: `
      <div class="ponte1-fluxo ponte1-reconstrucao">
        ...
        <p class="mini-jogo-subtitulo">Arrasta as peças para o sítio certo.</p>

        <div class="ponte1-tabuleiro" id="ponte1-tabuleiro">
          <img
            class="ponte1-fundo"
            src="assets/fundo-ponte1.png"
            alt="Zona de reconstrução da Ponte 1"
            style="left:${DESLOCAMENTO_VISUAL_PONTE1}%; top:${DESLOCAMENTO_VISUAL_PONTE1}%; width:${ESCALA_VISUAL_PONTE1 * 100}%; height:${ESCALA_VISUAL_PONTE1 * 100}%;"
          >
...
function verificarEncaixePecaPonte1(peca, estadoArrasto) {
  ...
  const dadosVisuais = obterDadosVisuaisPecaPonte1(dadosPeca);
  const atualX = parseFloat(peca.style.left) || 0;
  const atualY = parseFloat(peca.style.top) || 0;
  const distancia = Math.hypot(atualX - dadosVisuais.alvoX, atualY - dadosVisuais.alvoY);
...
function encaixarPecaPonte1(peca, estadoArrasto) {
  ...
  peca.style.left = `${dadosVisuais.alvoX}%`;
  peca.style.top = `${dadosVisuais.alvoY}%`;
  peca.style.width = `${dadosVisuais.largura}%`;
  ...
  if (estadoArrasto.encaixadas >= pecasReconstrucaoPonte1.length) {
    setTimeout(mostrarConfirmacaoFinalPonte1, 900);
  }
}
  ...
  abrirModal({
    ...
    conteudo: `
      <div class="ponte1-fluxo ponte1-confirmacao">
        <div class="ponte1-imagem-wrap ponte1-imagem-original">
          <img src="assets/original.png" alt="Mapa com a Ponte 1 construída">
        </div>
        <p class="ponte1-mensagem-final">A ponte voltou a unir as duas margens do rio.<br>Agora podes continuar a tua viagem e descobrir novas regiões.</p>
      </div>
    `,
    acoes: [
      {
...
const CAMINHO_BARCOS_RIO = ASSETS_JOGO.base;
const CAMINHO_MATERIAIS_BARCOS = ASSETS_JOGO.base;
  ...
  niveis: [
    {
      ...
      velocidadeMax: 26,
      corretos: [
        { id: "tabuas", nome: "Tábuas", imagem: caminhoAsset("madeira.png") },
        { id: "corda", nome: "Corda", imagem: caminhoAsset("corda.png") },
        { id: "pregos", nome: "Pregos", imagem: caminhoAsset("pregos.png") }
      ],
      errados: [
        { id: "peixe", nome: "Peixe", imagem: caminhoAsset("peixe.png") },
        { id: "maca", nome: "Maçã", imagem: caminhoAsset("maca.png") },
        { id: "bota", nome: "Bota", imagem: caminhoAsset("bota.png") }
      ]
    },
    {
      numero: 2,
      ...
      velocidadeMax: 34,
      corretos: [
        { id: "vigas-madeira", nome: "Vigas de madeira", imagem: caminhoAsset("vigas.png") },
        { id: "correntes", nome: "Correntes metálicas", imagem: caminhoAsset("correntes.png") },
        { id: "martelo", nome: "Martelo", imagem: caminhoAsset("martelo.png") }
      ],
      errados: [
        { id: "guarda-chuva", nome: "Guarda-chuva", imagem: caminhoAsset("guarda_chuva.png") },
        { id: "vaso-flor", nome: "Vaso com flor", imagem: caminhoAsset("vaso-flor.png") },
        { id: "bola", nome: "Bola", imagem: caminhoAsset("bola.png") }
      ]
    },
    {
      numero: 3,
      ...
      aceleracao: true,
      corretos: [
        { id: "candeeiro", nome: "Candeeiro", imagem: caminhoAsset("candeeiro.png") },
        { id: "caixa-ferramentas", nome: "Caixa de ferramentas", imagem: caminhoAsset("caixa-ferramentas.png") },
        { id: "cimento", nome: "Cimento", imagem: caminhoAsset("cimento.png") }
      ],
      errados: [
        { id: "frigideira", nome: "Frigideira", imagem: caminhoAsset("frigideira.png") },
        { id: "almofada", nome: "Almofada", imagem: caminhoAsset("almofada.png") },
        { id: "chapeu", nome: "Chapéu", imagem: caminhoAsset("chapeu.png") }
      ]
    }
  ]
};
...
function criarEstrelasErrosBarcosRio(erros) {
  return Array.from({ length: CONFIG_BARCOS_RIO.maxErros })
    .map((_, indice) => `<span class="estrela-erro coracao-erro ${indice < erros ? "apagada" : ""}">❤️</span>`)
    .join("");
}
    ...
    conteudo: `
      <div class="mini-jogo-barcos" data-nivel="${nivel.numero}">
        ...
        <section class="barcos-area-rio" id="barcos-area-rio" aria-label="Rio com barcos em movimento">
          ...
          <div class="barcos-mira escondida" id="barcos-mira" aria-hidden="true"></div>
        </section>
...
function criarBarcoRio() {
  ...
  botao.innerHTML = `
    <img class="barco-rio-img" src="assets/barco.png" alt="" aria-hidden="true">
  ...
  estadoBarcosRio.barcos.forEach((barco) => {
    ...
    if (barco.x > 122 || barco.x < -22) {
      ...
      if (barco.tipo === "correto") {
        processarMaterialUtilPerdidoBarcosRio(barco);
      }
...
function processarMaterialUtilPerdidoBarcosRio(barco) {
  ...
  escreverMensagemBarcosRio(`Deixaste passar ${barco.item.nome}.`);
  mostrarAvisoFlutuanteBarcosRio("Deixaste passar um material!");
  criarOndaErroBarcosRio(barco);
  destacarTentativasBarcosRio();
  atualizarHUDBarcosRio();
...
function mostrarAvisoFlutuanteBarcosRio(texto) {
  const area = document.getElementById("barcos-area-rio");
  if (!area) return;

  const avisoAnterior = area.querySelector(".barcos-aviso-flutuante");
  if (avisoAnterior) avisoAnterior.remove();

  if (estadoBarcosRio?.avisoTimeout) {
    clearTimeout(estadoBarcosRio.avisoTimeout);
    estadoBarcosRio.avisoTimeout = null;
  }

  const aviso = document.createElement("div");
  aviso.className = "barcos-aviso-flutuante";
  aviso.textContent = texto;
  area.appendChild(aviso);

  estadoBarcosRio.avisoTimeout = setTimeout(() => {
...
function destacarTentativasBarcosRio() {
  const chip = document.getElementById("barcos-tentativas")?.closest(".barcos-chip");
  if (!chip) return;

  chip.classList.remove("barcos-chip-alerta");
  void chip.offsetWidth;
  chip.classList.add("barcos-chip-alerta");

  setTimeout(() => chip.classList.remove("barcos-chip-alerta"), 520);
}
...
function prepararInputRatoBarcosRio() {
  const area = document.getElementById("barcos-area-rio");
  if (!area) return;

  const ativarRato = () => definirModoInputBarcosRio("rato");
  area.addEventListener("pointermove", ativarRato);
  area.addEventListener("pointerdown", ativarRato);
}

function definirModoInputBarcosRio(modo) {
  if (!estadoBarcosRio || estadoBarcosRio.modoInput === modo) return;

  estadoBarcosRio.modoInput = modo;
  atualizarMiraBarcosRio();
}
...
function moverMiraBarcosRio(tecla) {
  ...
  definirModoInputBarcosRio("teclado");
...
function atualizarMiraBarcosRio() {
  ...
  mira.classList.toggle("escondida", estadoBarcosRio.modoInput !== "teclado");
}
...
const CAMINHO_CARGAS_PERDIDAS = ASSETS_JOGO.base;
const CAMINHO_TRANSPORTES_CARGAS = ASSETS_JOGO.base;
  ...
  niveis: [
    {
      ...
      velocidadeMax: 20,
      faixas: [
        { y: 80, transporte: "vagoneta", direcao: "direita" }
      ],
      corretos: [
        { id: "vigas", nome: "Vigas", imagem: caminhoAsset("vigas.png") },
        { id: "correntes", nome: "Correntes", imagem: caminhoAsset("correntes.png") },
        { id: "martelo", nome: "Martelo", imagem: caminhoAsset("martelo.png") }
      ],
      errados: [
        { id: "peixe", nome: "Peixe", imagem: caminhoAsset("peixe.png") },
    ...
    },
    {
      ...
      velocidadeMax: 22,
      faixas: [
        { y: 80, transporte: "vagoneta", direcao: "esquerda" },
        { y: 80, transporte: "vagoneta", direcao: "direita" }
      ],
      corretos: [
        { id: "pregos", nome: "Pregos", imagem: caminhoAsset("pregos.png") },
        { id: "parafusos", nome: "Parafusos", imagem: caminhoAsset("parafusos.png") },
        { id: "alicate", nome: "Alicate", imagem: caminhoAsset("alicate.png") }
      ],
      errados: [
        { id: "chapeu", nome: "Chapéu", imagem: caminhoAsset("chapeu.png") },
        { id: "flores", nome: "Flores", imagem: caminhoAsset("flores.png") },
        { id: "bolo", nome: "Bolo", imagem: caminhoAsset("bolo.png") }
      ]
    },
    {
      numero: 3,
      nome: "Carga III",
      mensagemInicial: "Última carga: separa o que vem na vagoneta ou na plataforma suspensa.",
      ...
      faixas: [
        { y: 80, transporte: "vagoneta", direcao: "direita" },
        { y: 14, transporte: "plataforma", direcao: "esquerda" },
        { y: 80, transporte: "vagoneta", direcao: "esquerda" },
        { y: 14, transporte: "plataforma", direcao: "direita" }
      ],
      corretos: [
        { id: "troncos", nome: "Troncos", imagem: caminhoAsset("troncos.png") },
        { id: "corda", nome: "Corda", imagem: caminhoAsset("corda.png") },
        { id: "madeira", nome: "Madeira", imagem: caminhoAsset("madeira.png") }
      ],
      errados: [
        { id: "bola", nome: "Bola", imagem: caminhoAsset("bola.png") },
        { id: "pena", nome: "Pena", imagem: caminhoAsset("pena.png") },
        { id: "guarda_chuva", nome: "Guarda-chuva", imagem: caminhoAsset("guarda_chuva.png") }
      ]
    }
  ]
};
      ...
      <div class="mini-cargas" data-nivel="${nivel.numero}">
        <header class="mini-cargas-cabecalho">
          <div class="mini-cargas-info">
            ...
            <p class="mini-cargas-regra">${nivel.numero === 3 ? "Quando a vagoneta ou a plataforma passar, arrasta apenas a carga para Construção ou Descarte." : "Quando a vagoneta passar, arrasta apenas a carga para Construção ou Descarte."}</p>
          </div>
        ...
        <section class="mini-cargas-area" id="mini-cargas-area" aria-label="Oficina de triagem de cargas">
          <img class="mini-cargas-fundo" src="${nivel.numero === 3 ? "assets/oficina03_mod_nivel3_02.png" : "assets/oficina03_mod.png"}" alt="" aria-hidden="true">
...
function mostrarProximaCargaCargasPerdidas() {
  ...
  estadoCargasPerdidas.cargaAtual = {
    ...
    y: faixa.y || 70,
    velocidade: gerarVelocidadeCargasPerdidas(estadoCargasPerdidas.nivel, faixa.transporte || "vagoneta"),
    transporte: faixa.transporte || "vagoneta",
  };

  const transporteAtual = estadoCargasPerdidas.cargaAtual.transporte || "vagoneta";
  const veiculo = document.createElement("div");
  veiculo.className = `mini-cargas-veiculo mini-cargas-veiculo-${transporteAtual}`;
  const imagemTransporte = transporteAtual === "plataforma" ? "assets/plataforma.png" : "assets/vagoneta.png";
  veiculo.innerHTML = `<img class="mini-cargas-transporte" src="${imagemTransporte}" alt="" aria-hidden="true">`;

  const botao = document.createElement("button");
  botao.type = "button";
  botao.className = `mini-cargas-carga-item mini-cargas-${carga.tipo} mini-cargas-carga-${transporteAtual}`;
...
function gerarVelocidadeCargasPerdidas(nivel, transporte = "vagoneta") {
  const min = nivel.velocidadeMin || 6;
  const max = nivel.velocidadeMax || 9;
  const velocidade = min + Math.random() * (max - min);
  return transporte === "plataforma" ? velocidade * 0.68 : velocidade;
}
...
function posicionarVeiculoCargasPerdidas() {
  ...
  carga.veiculo.classList.toggle("transporte-suspenso", carga.transporte === "plataforma");
}
...
function iniciarMovimentoCargaAtualCargasPerdidas() {
  ...
  const animar = (tempo) => {
    ...
    posicionarVeiculoCargasPerdidas();
      ...
      const margemSaida = carga.transporte === "plataforma" ? 108 : 112;
    const margemEntrada = carga.transporte === "plataforma" ? -8 : -12;
    const saiu = carga.direcao === "direita" ? carga.x > margemSaida : carga.x < margemEntrada;
...
function atualizarDestaqueBaseZonasCargasPerdidas(ativo) {
  document.querySelectorAll(".mini-cargas-zona").forEach((zona) => {
    zona.classList.toggle("destacada", ativo);
  });
}

function destacarZonaCargasPerdidas(clientX, clientY) {
  atualizarDestaqueBaseZonasCargasPerdidas(true);
...
function limparDestaqueZonasCargasPerdidas() {
  document.querySelectorAll(".mini-cargas-zona").forEach((zona) => { zona.classList.remove("ativa"); zona.classList.remove("destacada"); });
}
...
const CAMINHO_PONTE2 = ASSETS_JOGO.base;
    ...
    conteudo: `
      <div class="fluxo2-mini-jogo" aria-label="Mini-jogo Reconstruir o Fluxo">
        <div class="fluxo2-cenario">
          <img class="fluxo2-fundo" src="assets/ponte-inativa.png" alt="Ponte 2 adormecida">
          <img class="fluxo2-fundo fluxo2-fundo-ativa" src="assets/ponte-ativa.png" alt="" aria-hidden="true">

          <header class="fluxo2-cabecalho">
            <div class="fluxo2-texto-cabecalho">
...
function criarHTMLPecaFluxo2(peca) {
  ...
  if (peca.tipo === "fonte") {
    interior = `<img class="fluxo2-asset fluxo2-fonte" src="assets/cristal-central.png" alt="">`;
  } else if (peca.tipo === "lanterna") {
    const acesa = estadoFluxoPonte2?.lanternasAcesas?.includes(peca.id);
    interior = `<img class="fluxo2-asset fluxo2-lanterna" src="${caminhoAsset(acesa ? "lanterna-ativa.png" : "lanterna-inativa.png")}" alt="">`;
  } else {
    interior = `
      <img class="fluxo2-base-runa" src="assets/runa-magica.png" alt="">
...
function aplicarEstadoVisualFluxo2() {
  ...
  document.querySelectorAll(".fluxo2-peca").forEach((elemento) => {
    ...
    if (peca && peca.tipo === "lanterna") {
      ...
      if (imagemLanterna) {
        imagemLanterna.src = caminhoAsset(lanternaAcesa ? "lanterna-ativa.png" : "lanterna-inativa.png");
      }
    }
...
function terminarFluxoPonte2ComSucesso() {
  ...
  if (cenario) {
    cenario.classList.add("fluxo2-mostrar-ponte-final");

    const efeito = document.createElement("img");
    efeito.className = "fluxo2-efeito-sucesso";
    efeito.src = caminhoAsset("efeito-sucesso.png");
    efeito.alt = "";
...
function iniciarMiniJogoUltimaTravessia({ ponte = "ponte3" } = {}) {
  const CAMINHO_PONTE3 = ASSETS_JOGO.base;

  const imagensPonte = {
    0: caminhoAsset("ponte3_estado_0.png"),
    1: caminhoAsset("ponte3_estado_1.png"),
    2: caminhoAsset("ponte3_estado_2.png"),
    final: caminhoAsset("ponte3_estado_final.png")
  };
  ...
  const DEFINICOES_ITENS = {
    caixa_materiais: {
      ...
      imagem: "assets/caixa-fechada.png",
      gerador: true
    },

    corda: { tipo: "corda", nome: "Corda", imagem: caminhoAsset("corda.png") },
    corrente_pesada: { tipo: "corrente_pesada", nome: "Correntes", imagem: caminhoAsset("correntes.png") },
    cabo_suspensao: { tipo: "cabo_suspensao", nome: "Cabo de suspensão", imagem: caminhoAsset("cabo-supenso.png") },
    plataforma: { tipo: "plataforma", nome: "Plataforma", imagem: caminhoAsset("plataforma.png") },
    suporte_pesado: { tipo: "suporte_pesado", nome: "Suporte pesado", imagem: caminhoAsset("suporte_pesado.png"), maximo: true },

    pregos: { tipo: "pregos", nome: "Pregos", imagem: caminhoAsset("pregos.png") },
    parafusos: { tipo: "parafusos", nome: "Parafusos", imagem: caminhoAsset("parafusos.png") },
    alicates: { tipo: "alicates", nome: "Alicates", imagem: caminhoAsset("alicate.png") },
    caixa_ferramentas: { tipo: "caixa_ferramentas", nome: "Caixa de ferramentas", imagem: caminhoAsset("caixa-ferramentas.png") },
    candeeiro: { tipo: "candeeiro", nome: "Candeeiro", imagem: caminhoAsset("candeeiro.png"), maximo: true },

    troncos: { tipo: "troncos", nome: "Troncos", imagem: caminhoAsset("troncos.png") },
    madeira: { tipo: "madeira", nome: "Madeira", imagem: caminhoAsset("madeira.png") },
    vigas: { tipo: "vigas", nome: "Vigas", imagem: caminhoAsset("vigas.png") },
    estrutura_reforcada: { tipo: "estrutura_reforcada", nome: "Estrutura reforçada", imagem: caminhoAsset("estrutura_reforcada.png"), maximo: true },

    runa_magica: { tipo: "runa_magica", nome: "Runa mágica", imagem: caminhoAsset("runa-magica.png") },
    cristal_pequeno: { tipo: "cristal_pequeno", nome: "Cristal pequeno", imagem: caminhoAsset("cristal-pequeno.png") },
    cristal_central: { tipo: "cristal_central", nome: "Cristal central", imagem: caminhoAsset("cristal-central.png") },
    mecanismo_magico: { tipo: "mecanismo_magico", nome: "Mecanismo mágico", imagem: caminhoAsset("mecanismo-magico.png") },
    nucleo_magico: { tipo: "nucleo_magico", nome: "Núcleo mágico", imagem: caminhoAsset("nucleo_magico.png"), maximo: true },

    secao_ponte: { tipo: "secao_ponte", nome: "Secção da ponte", imagem: caminhoAsset("secao_ponte.png"), especial: true },
    energia_ativa: { tipo: "energia_ativa", nome: "Energia ativa", imagem: caminhoAsset("energia_ativa.png"), especial: true }
  };
  ...
  const estadoMerge = {
    ...
    slotsComErro: [],
    mensagem: "Junta duas peças iguais para evoluir. No fim, junta peças diferentes compatíveis para concluir a ponte."
  };
  ...
  function produzirMaterial() {
    ...
    if (!tipo) {
      ...
      estadoMerge.slotsComErro = [0];
      estadoMerge.mensagem = "Agora junta peças diferentes compatíveis: estrutura reforçada + suporte pesado, candeeiro + núcleo mágico, e energia ativa + secção da ponte.";
      tocarSom("erro");
      renderizarMiniJogo();
      return;
    }
  ...
  function slotEhCompativelComItem(slot, itemOrigem, origemId = null) {
    if (!slot || !itemOrigem || origemId === slot.id) return false;
    if (!slot.item || slot.item.gerador || itemOrigem.gerador) return false;
    return Boolean(obterReceita(itemOrigem, slot.item));
  }

  function classeSlot(slot) {
    const classes = ["ponte3-celula", "ponte3-opcao"];
    const slotSelecionado = estadoMerge.selecionado !== null
      ? estadoMerge.slots[estadoMerge.selecionado]
      : null;
    ...
    if (slotEhCompativelComItem(slot, slotSelecionado?.item, estadoMerge.selecionado)) {
      classes.push("compativel-selecao");
    }
        ...
        <section class="ponte3-mini-jogo ponte3-merge-board ${animacaoFinal ? "ponte3-com-animacao-final" : ""}" aria-labelledby="ponte3-titulo">
          <div class="ponte3-cenario">
            ...
            <header class="ponte3-cabecalho-board">
              ...
              <p>Chegaste ao último desafio. Constrói a ponte final para voltar a ligar todas as regiões.</p>
            </header>
            ...
            <div class="ponte3-painel-jogo">
              <div class="ponte3-info-lateral">
                ...
                <p class="ponte3-dica"><strong>Como jogar</strong><br>Junta duas peças iguais para as fazer evoluir.<br><br>Quando tiveres as peças finais, combina:<br>• Estrutura + Suporte<br>• Candeeiro + Núcleo<br>• Energia + Secção</p>
              </div>
  ...
  function limparDestaquesCompatibilidadePonte3() {
    document.querySelectorAll(".ponte3-celula.compativel-drag, .ponte3-celula.destino-compativel").forEach((celula) => {
      celula.classList.remove("compativel-drag", "destino-compativel");
    });
  }

  function marcarDestaquesCompatibilidadePonte3(origemId) {
    limparDestaquesCompatibilidadePonte3();

    const origem = estadoMerge.slots[origemId];
    if (!origem?.item || origem.item.gerador) return;

    document.querySelectorAll(".ponte3-celula").forEach((celula) => {
      const slotId = Number(celula.dataset.slot);
      const slotDestino = estadoMerge.slots[slotId];
      if (slotEhCompativelComItem(slotDestino, origem.item, origemId)) {
        celula.classList.add("compativel-drag");
      }
    });
  }
  ...
  function ligarEventosTabuleiro() {
    document.querySelectorAll(".ponte3-celula").forEach((celula) => {
      ...
      celula.addEventListener("dragstart", (evento) => {
        ...
        marcarDestaquesCompatibilidadePonte3(slotId);
      });
      ...
      celula.addEventListener("dragover", (evento) => {
        ...
        const origemId = estadoMerge.arrastado;
        const origem = estadoMerge.slots[origemId];
        const destino = estadoMerge.slots[slotId];
        if (slotEhCompativelComItem(destino, origem?.item, origemId)) {
          celula.classList.add("destino-compativel");
        }
      });

      celula.addEventListener("dragleave", () => {
        celula.classList.remove("destino-drag", "destino-compativel");
      });
        ...
        <section class="ponte3-mini-jogo ponte3-final-board ponte3-final-limpo ponte3-final-sem-ui" aria-labelledby="ponte3-vitoria-titulo">
          <div class="ponte3-final-palco-limpo" style="background-image: url('${imagensPonte.final}');" role="img" aria-label="Ponte final construída e iluminada">
            <div class="ponte3-mensagem-final" aria-live="polite">
              <h2 id="ponte3-vitoria-titulo">A última ponte foi reconstruída!</h2>
              <p>A última ponte está concluída. Todas as regiões voltaram a estar ligadas.</p>
