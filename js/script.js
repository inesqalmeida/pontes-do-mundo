...
const STORAGE_BACKUP_KEY_V3 = `${STORAGE_KEY_V3}_backup`;
...
const elementos = {
  ...
  tituloPerfil: document.querySelector("#ecra-perfil h2"),
...
const sons = {
  ...
  hover: new Audio("assets/hover.ogg"),

  sucesso: new Audio("assets/sucesso.mp3"),
  erro: new Audio("assets/erro.mp3"),
  erroForte: new Audio("assets/erro_forte.mp3"),

  ponte: new Audio("assets/ponte.mp3"),
  inicio: new Audio("assets/inicio.mp3"),

  musicaFundo: new Audio("assets/musica_fundo.mp3")
};
...
function armazenamentoDisponivel(tipo = "localStorage") {
  try {
    const armazenamento = window[tipo];
    const chaveTeste = `pontes_do_mundo_teste_${Date.now()}`;
    armazenamento.setItem(chaveTeste, "1");
    armazenamento.removeItem(chaveTeste);
    return true;
  } catch (erro) {
    console.warn(`${tipo} indisponível. O jogo continua, mas sem persistência completa.`, erro);
    return false;
  }
}

function lerStorageSeguro(chave, tipo = "localStorage") {
  try {
    return window[tipo].getItem(chave);
  } catch (erro) {
...
function gravarStorageSeguro(chave, valor, tipo = "localStorage") {
  try {
    window[tipo].setItem(chave, valor);
    return true;
  } catch (erro) {
...
function removerStorageSeguro(chave, tipo = "localStorage") {
  try {
    window[tipo].removeItem(chave);
    return true;
  } catch (erro) {
    console.warn(`Não foi possível remover ${chave}.`, erro);
    return false;
  }
}

function guardarEstado() {
  const estadoSeguro = normalizarEstadoGuardado(estado, { limparTemporario: false });
  estadoSeguro.versaoSave = SAVE_VERSION;
  estado = estadoSeguro;

  const payload = JSON.stringify(estadoSeguro);
  const guardado = gravarStorageSeguro(STORAGE_KEY_V3, payload);

  if (guardado) {
    gravarStorageSeguro(STORAGE_BACKUP_KEY_V3, payload);
  }
...
function chavePonteValida(chave) {
  return ["ponte1", "ponte2", "ponte3"].includes(chave);
}

function chaveZonaValida(chave) {
  return ["sabores", "vozes", "coragem", "oficina", "ponte1", "ponte2", "ponte3"].includes(chave);
}

function normalizarPerguntaPendente(area, valor) {
  if (!valor || typeof valor !== "object" || Array.isArray(valor)) return null;

  return {
    id: textoSeguro(valor.id, ""),
    texto: textoSeguro(valor.texto, ""),
    corretaId: textoSeguro(valor.corretaId, ""),
    opcoesTextos: Array.isArray(valor.opcoesTextos)
      ? valor.opcoesTextos.map((opcao) => textoSeguro(opcao, "")).filter(Boolean).slice(0, 8)
      : []
  };
}

function normalizarEstadoTemporario(dados, normalizado) {
  normalizado.modalAberto = Boolean(dados.modalAberto);
  normalizado.zonaAtual = chaveZonaValida(dados.zonaAtual) ? dados.zonaAtual : null;

  if (
    dados.construcaoEmRisco &&
    typeof dados.construcaoEmRisco === "object" &&
    chavePonteValida(dados.construcaoEmRisco.ponte)
  ) {
    normalizado.construcaoEmRisco = {
      ponte: dados.construcaoEmRisco.ponte,
      ativo: Boolean(dados.construcaoEmRisco.ativo)
    };
  } else {
    normalizado.construcaoEmRisco = null;
  }

  normalizado.perguntaPendente = {
    sabores: normalizarPerguntaPendente("sabores", dados.perguntaPendente?.sabores),
    vozes: normalizarPerguntaPendente("vozes", dados.perguntaPendente?.vozes),
    coragem: normalizarPerguntaPendente("coragem", dados.perguntaPendente?.coragem)
  };
}
...
function limparMemoriaTemporariaMiniJogos() {
  try {
    removerStorageSeguro(SESSION_KEY_CAIXAS_PERDIDAS, "sessionStorage");
  } catch (erro) {
...
function normalizarEstadoGuardado(dados, opcoes = {}) {
  ...
  // Estes dados são seguros apenas durante a sessão actual.
  // Ao carregar o jogo novamente, devem ser limpos para evitar modais, mini-jogos ou perguntas presas.
  if (opcoes.limparTemporario === false) {
    normalizarEstadoTemporario(dados, normalizado);
  } else {
...
function atualizarIdentidadePerfil(nome, avatarSelecionado) {
  estado.nomeJogador = textoSeguro(nome).trim();
  estado.avatarSelecionado = textoSeguro(avatarSelecionado);
...
function prepararEcraPerfil({ edicao = false } = {}) {
  perfilEmEdicao = Boolean(edicao);
  atualizarPerfil();

  if (elementos.tituloPerfil) {
    elementos.tituloPerfil.textContent = perfilEmEdicao ? "Edita o teu explorador" : "Escolhe o teu explorador";
  }

  elementos.botaoComecar.textContent = perfilEmEdicao ? "Guardar alterações" : "Começar";
...
function carregarEstado() {
  armazenamentoDisponivel("localStorage");
  armazenamentoDisponivel("sessionStorage");

  const tentarCarregar = (chave, descricao) => {
    const guardado = lerStorageSeguro(chave);
    ...
    try {
      ...
      console.error(`Não foi possível carregar ${descricao}:`, erro);
      removerStorageSeguro(chave);
      return false;
    }
  };

  if (tentarCarregar(STORAGE_KEY_V3, "o progresso principal")) {
    return true;
  }

  if (tentarCarregar(STORAGE_BACKUP_KEY_V3, "a cópia de segurança do progresso")) {
    return true;
  }

  const guardadoV1 = lerStorageSeguro(STORAGE_KEY_V1);
  if (guardadoV1) {
    try {
      estado = normalizarEstadoGuardado(migrarEstadoV1(JSON.parse(guardadoV1)));
      guardarEstado();
      removerStorageSeguro(STORAGE_KEY_V1);
      return true;
    } catch (erro) {
...
function reiniciarJogoCompleto() {
  ...
  removerStorageSeguro(STORAGE_KEY_V3);
  removerStorageSeguro(STORAGE_BACKUP_KEY_V3);
  removerStorageSeguro(STORAGE_KEY_V1);
  limparMemoriaTemporariaMiniJogos();
...
function abrirModal({ etiqueta = "", titulo = "", conteudo = "", acoes = [], mostrarFechar = true }) {
  ...
  acoes.forEach((acao) => {
    ...
    if (acao.secundario) {
      botao.classList.add("botao-secundario");
    } else {
      botao.classList.add("botao-acao-principal");
    }
...
function focarPrimeiroBotaoModal() {
  ...
  const existePerguntaComOpcoes = Boolean(
    elementos.modalOverlay.querySelector(".pergaminho-pergunta-area .modal-opcao:not(:disabled), .modal-opcoes .modal-opcao:not(:disabled)")
  );

  /*
    Nas perguntas, não focamos automaticamente a primeira resposta para não parecer
    que uma opção já foi escolhida. Nos restantes modais, o foco deve ir para a
    primeira ação principal, para o Enter confirmar a escolha mais lógica.
  */
  if (!existePerguntaComOpcoes) {
    const primeiraAcao = elementos.modalOverlay.querySelector(
      ".pergaminho-acoes button:not(:disabled), .modal-acoes button:not(:disabled)"
    );

    if (primeiraAcao) {
      primeiraAcao.focus({ preventScroll: true });
      return;
    }
  }
...
function mostrarTutorialInicialDoMapa() {
  ...
  abrirModal({
    ...
    conteudo: criarPergaminhoNarrativoHTML({
      ...
      conteudo: `
        <p>Bem-vindo ao mundo de Mosaico.</p><p>As pontes que ligavam as diferentes regiões foram destruídas.</p><p>Explora cada região, aprende coisas novas e ajuda a reconstruir as pontes para voltar a ligar pessoas e culturas.</p><p><strong>Boa aventura!</strong></p>
      `
    }),
    acoes: [
      {
...
function verificarZonaAtual() {
  ...
  if (estado.zonaAtual) {
    ...
    if (estado.zonaAtual === "sabores") esconderGuiaSabores();
    else atualizarGuiaSabores();
...
function concluirConstrucaoPonte(ponte, feedbackEl) {
  ...
  setTimeout(() => {
    ...
    if (ponte === "ponte1") {
      abrirModal({
        etiqueta: "✨",
        titulo: "Ponte reconstruída!",
        conteudo: "<p>A ponte voltou a unir as duas margens do rio.</p><p>Agora podes continuar a tua viagem e descobrir novas regiões.</p>",
    ...
    if (ponte === "ponte2") {
      abrirModal({
        etiqueta: "✨",
        titulo: "Ponte reconstruída!",
        conteudo: "<p>Mais um caminho foi recuperado. As ligações entre regiões estão a ficar mais fortes.</p>",
        acoes: [{ texto: "Continuar", onClick: fecharModal }]
      });
      return;
    }
    ...
    abrirModal({
      ...
      conteudo: "<p>As pontes voltaram a ligar caminhos, pessoas e culturas.<br>Graças à tua ajuda, o mundo está novamente unido.</p>",
      acoes: [
        {
...
function abrirConstrucaoPonte(ponte) {
  ...
  const textoConstrucao = ponte === "ponte3"
    ? `<p>Fantástico! Já tens os materiais necessários para reconstruir a última ponte.</p><p>Este é o desafio final.</p><p>Se venceres, todas as regiões voltarão a ficar ligadas.</p><p><strong>Boa sorte!</strong></p>`
    : `<p>Ótimo! Já tens os materiais necessários para construir esta ponte.</p><p>Vais usar esses materiais para enfrentar o desafio da ponte.</p><p>Se venceres, mais uma região ficará ligada ao resto do mundo.</p><p><strong>Boa sorte!</strong></p>`;

  abrirModal({
    etiqueta: "🌉",
    titulo: `Construir ${nomePonte(ponte)}`,
    conteudo: textoConstrucao,
    acoes: [
      {
        texto: "Aceitar desafio",
        onClick: () => {
...
function configurarInterface() {
  ...
  elementos.botaoJogar.addEventListener("click", () => {
    tocarSom("click");
    iniciarMusicaFundo();
    prepararEcraPerfil({ edicao: false });
    mostrarEcra("perfil");
  });
  ...
  elementos.botaoComecar.addEventListener("click", () => {
    ...
    if (perfilEmEdicao) {
      atualizarIdentidadePerfil(nome, avatarSelecionado);
      elementos.mensagemErro.textContent = "";
  ...
  elementos.botaoVoltarPerfil.addEventListener("click", () => {
    tocarSom("click");
    fecharPainelDefinicoes();
    prepararEcraPerfil({ edicao: true });
    mostrarEcra("perfil");
  });
...
function mostrarDebugCoordenadas() {
  ...
  debugBox.textContent = `x: ${estado.personagem.x.toFixed(1)} | y: ${estado.personagem.y.toFixed(1)} | zona: ${estado.zonaAtual}`;
}
